			      ...
			      unsigned int request, void *arg)
{
	...
	struct rtdm_gpio_pin *pin;
	printk("[%s] gpio=%d request=%d \n", __FUNCTION__, gpio, request);
			  ...
			  int type)
{
	...
	for (;;) {
		...
		if (np == NULL)
			break;
		printk("rtdm gpio: found of node\n");
		pdev = of_find_device_by_node(np);
		of_node_put(np);
		if (pdev == NULL)
			break;
		printk("rtdm gpio: found of pdev\n");
