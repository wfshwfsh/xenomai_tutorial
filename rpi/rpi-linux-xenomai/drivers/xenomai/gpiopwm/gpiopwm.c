...
static inline int gpiopwm_config(struct rtdm_fd *fd, struct gpiopwm *conf)
{
	...
	printk("[%s] gpio=%d, duty=%d, base_period=%d, period=%d \n"
		, __FUNCTION__, conf->gpio, conf->duty_cycle, conf->period, ctx->duty.period);
...
static inline int gpiopwm_change_duty_cycle(struct gpiopwm_priv *ctx, unsigned int cycle)
{
	...
	printk("[%s] cycle=%d, period=%d \n"
		, __FUNCTION__, cycle, duty_period(&ctx->ctrl.duty));
