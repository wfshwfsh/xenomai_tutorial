			    ...
			    void *rx, size_t len)
{
	...
	printk("%s\t rx_len=%d rx_data:", __FUNCTION__, len);
	for(i=0;i<len;i++)
	{
		printk("%d ", spim->rx_buf[i]);
	}
	printk("\n\n");
			     ...
			     const void *tx, size_t len)
{
	...
	printk("%s\t tx_len=%d, tx_data:", __FUNCTION__, len);
        for(i=0;i<len;i++)
        {
                printk("%d ", spim->tx_buf[i]);
        }
        printk("\n\n");
...
static int set_iobufs(struct spi_slave_bcm2835 *bcm, size_t len)
{
	 ...
	 */
	p = dma_alloc_coherent(bcm->dma_dev, len, &dma, GFP_KERNEL);
	if (p == NULL)
		return -ENOMEM;
			       ...
			       struct vm_area_struct *vma)
{
	...
	//ret = dma_mmap_coherent(bcm->dma_dev, vma, bcm->io_virt, bcm->io_dma, length);

	//if (ret < 0) {
	//	printk(  "mmap_alloc: remap failed (%d)\n", ret);
	//	return ret;
	//}
...
static void bcm2835_mmap_release(struct rtdm_spi_remote_slave *slave)
{
	...
	dma_free_coherent(bcm->dma_dev, bcm->io_len,
...
bcm2835_attach_slave(struct rtdm_spi_master *master, struct spi_device *spi)
{
	...
	{
		dev = master_to_kdev(master);
		bcm->dma_dev = get_device(dev);
	
		/*
       		 * set this->dma_dev->dma_mask
         	 */
        	if (bcm->dma_dev->dma_mask == NULL) {
	            bcm->dma_dev->dma_mask = &bcm->dma_dev->coherent_dma_mask;
	        }
        	/*
	         ...
	         */
        	if (*bcm->dma_dev->dma_mask == 0) {
	            int retval = dma_set_mask_and_coherent(bcm->dma_dev, DMA_BIT_MASK(dma_mask_bit));
	            if (retval != 0) {
	                //printk(KERN_WARNING "dma_set_mask_and_coherent(DMA_BIT_MASK(%d)) failed. return=(%d)\n", dma_mask_bit, retval);
	                *bcm->dma_dev->dma_mask         = DMA_BIT_MASK(dma_mask_bit);
	                bcm->dma_dev->coherent_dma_mask = DMA_BIT_MASK(dma_mask_bit);
	            }
	        }
	}
