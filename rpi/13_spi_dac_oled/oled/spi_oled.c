...
long OLED_DIsp_Set_Pos(int x, int y)
{
	unsigned long arg;
	arg = ((y & 0xff) << 8) | (x & 0xff);
	
	return ioctl(fd_spidev, OLED_IOC_SET_POS, arg);
}
...
{      			    
    ...
    for(t=0;t<16;t++)
	{//显示上半截字符	
		oled_write_datas(&hz_1616[no][t*2],OLED_DATA);
		adder+=1;
    }	
	OLED_DIsp_Set_Pos(x,y+1);	
    for(t=0;t<16;t++)
	{//显示下半截字符
		oled_write_datas(&hz_1616[no][t*2+1],OLED_DATA);
		adder+=1;
    }					
}
